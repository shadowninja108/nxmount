...
namespace nxmount.Util
{
    public class ShallowFileSystemWrapper(IFileSystem impl) : IFileSystem
    {
        ...
        protected override Result DoCreateFile(in Path path, long size, CreateFileOptions option) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoDeleteFile(in Path path) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoCreateDirectory(in Path path) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoDeleteDirectory(in Path path) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoDeleteDirectoryRecursively(in Path path) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoCleanDirectoryRecursively(in Path path) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoRenameFile(in Path currentPath, in Path newPath) => ResultFs.WriteUnpermitted.Value;

        protected override Result DoRenameDirectory(in Path currentPath, in Path newPath) => ResultFs.WriteUnpermitted.Value;
